...
const Home = async () => {
  ...
  return (
    <section className="sm:p-16 py-16 px-8 flex flex-col gap-10">
      <h2 className="text-3xl text-white font-bold">Аниме <span className="red-gradient">アニメ</span></h2>

      <section className="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-10 items-start self-center">
        {animes.slice(0, 7).map((item: animeType, index: number) => (
      ...
      <h2 className="text-3xl text-white font-bold">Манга <span className="red-gradient">マンガ</span></h2>

      <section className="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-10 items-start self-center">
        {mangas.slice(0, 7).map((item: mangaType, index: number) => (
