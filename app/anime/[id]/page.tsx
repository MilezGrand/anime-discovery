...
const AnimeCardPage = async ({ params }: AnimeCardPageProps) => {
  const id = params.id;
  const data: IAnime = await fetchAnime(id);
      ...
      <div>
        <section className='sm:grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 flex flex-col gap-10'>
          ...
          <ul>
            ...
            <li className='text-gray-400 pb-2'>Даты: <span className='text-white'>с {airedDate} по {releasedDate}</span></li>
            <li className='text-gray-400 pb-2'>Жанры: <span className='text-white'>{data.genres.map((genre) => (<Link href={`/genre/${genre.id}`} key={genre.id} className={`hover:text-blue-500 underline pr-1`}>{genre.russian}</Link>))}</span></li>
