...
async function Home() {
  ...
  return (
    <section className="sm:p-16 py-16 px-8 flex flex-col gap-10">
      <Link href='/anime'><h2 className="text-3xl text-white font-bold">Аниме</h2></Link>
      ...
      <section className="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-10 jus">
        ...
        <Link href='/anime'><div className="border-2 border-slate-700  hover:bg-slate-700 text-xl rounded-lg p-4 h-[37vh] flex justify-center items-center" >Еще</div></Link>
      </section>

      <Link href='/manga'><h2 className="text-3xl text-white font-bold">Манга</h2></Link>

      <section className="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-10 jus">
        {mangas.slice(0, 7).map((item: MangaProp, index: number) => (
          <AnimeCard key={item.id} manga={item} index={index} />
        ))}
        <Link href='/manga'><div className="border-2 border-slate-700  hover:bg-slate-700 text-xl rounded-lg p-4 h-[37vh] flex justify-center items-center" >Еще</div></Link>
      </section>
    </section >
  );
}
